...
class Reg
{
	...
	public static function register($email, $username, $pass)
	{
		global $config;
		$bdd = new BDD();
		$bdd->insert("users", "'', '".$bdd->real_escape_string($username)."', '".$bdd->real_escape_string($email)."', '".sha1($pass)."', '', '', '', '".tps()."', '".$_SERVER['REMOTE_ADDR']."', '".$_SERVER['REMOTE_ADDR']."', '".$config['rank_mbr']."'") or die(mysql_error());
		$bdd->close();
	}
}
?>